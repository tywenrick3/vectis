...
async function stageAnalytics(state: PipelineState): Promise<void> {
  const data = await apiCall("POST", "/pipeline/analytics", {}, 300_000);
...
async function stageResearch(state: PipelineState): Promise<void> {
  const data = await apiCall("POST", "/pipeline/research", { niche: state.niche }, 480_000);
...
async function stageAssemble(state: PipelineState): Promise<void> {
  const data = await apiCall(
    ...
    "/pipeline/assemble",
    {
      ...
      // Variants are still generated by ideation; flip to true to render them.
      include_hook_variants: false,
    },
    1_200_000, // 20 min timeout
  );
